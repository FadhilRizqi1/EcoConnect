...
class MainShell extends StatelessWidget {
  ...
  static final ValueNotifier<bool> _navigationHidden = ValueNotifier(false);

  static void setNavigationHidden(bool hidden) {
    if (_navigationHidden.value != hidden) {
      _navigationHidden.value = hidden;
    }
  }
  ...
  static const _rightTabs = [
    _TabItem(
        icon: LucideIcons.trophy, label: 'Peringkat', path: '/papan-peringkat'),
  ...
  int _currentIndex(BuildContext context, List<_TabItem> tabs) {
    ...
    for (int i = 0; i < tabs.length; i++) {
      ...
      if (location.startsWith(tabs[i].path) && tabs[i].path != '/profil')
        return i;
    }
    return -1;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isFabActive =
        GoRouterState.of(context).matchedLocation.startsWith('/tugas');
    final isDark = Theme.of(context).brightness == Brightness.dark;

    return ValueListenableBuilder<bool>(
      valueListenable: _navigationHidden,
      builder: (context, navigationHidden, _) {
        return Scaffold(
          backgroundColor: Theme.of(context).scaffoldBackgroundColor,
          extendBody:
              true, // This allows the body to flow underneath the BottomAppBar, filling the notch area
          body: child,
          // Fitts's Law: Core action (Check-in) is prominent and easy to reach.
          floatingActionButtonLocation:
              FloatingActionButtonLocation.centerDocked,
          floatingActionButton: navigationHidden
              ? null
              : Container(
                  height: 64,
                  width: 64,
                  decoration: BoxDecoration(
                    ...
                    gradient: const LinearGradient(
                      colors: [
                        Color(0xFF4FC87A),
                        Color(0xFF1A4D2E)
                      ], // Dominant Premium Green
                      begin: Alignment.topLeft,
                      end: Alignment.bottomRight,
                    ),
                    border: Border.all(
                      color: const Color(0xFFFFB03A)
                          .withOpacity(0.9), // Orange 'lis' (border)
                      width: 2.5,
                    ),
                    boxShadow: [
                      BoxShadow(
                        color: const Color(0xFFFFB03A)
                            .withOpacity(0.4), // Orange 'cahaya' (glow)
                        blurRadius: 16,
                        spreadRadius: 2,
                        offset: const Offset(
                            0, 0), // Centered glow for uniform light
                      ),
                    ],
                  ),
          ...
          bottomNavigationBar: navigationHidden
              ? null
              : BottomAppBar(
                  ...
                  child: SizedBox(
                    height:
                        68, // Ditambah sedikit agar tidak ada vertical overflow
                    child: Row(
                        ...
                        Expanded(
                          child: Row(
                            children: List.generate(
                                _leftTabs.length,
                                (i) => _buildTabItem(
                                    context, _leftTabs[i], i == leftIndex)),
                          ),
                        ),
                        ...
                        Expanded(
                          child: Row(
                            children: List.generate(
                                _rightTabs.length,
                                (i) => _buildTabItem(
                                    context, _rightTabs[i], i == rightIndex)),
                          ),
                        ),
                      ],
                    ),
  ...
  Widget _buildTabItem(BuildContext context, _TabItem tab, bool isActive) {
    ...
    final inactiveColor =
        isDark ? AppColors.textMuted : const Color(0xFF9E9E9E);
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          padding: const EdgeInsets.symmetric(
              vertical: 4), // Dihapus horizontal padding agar fit
          decoration: BoxDecoration(
            color: isActive
          ...
          child: Column(
            ...
            children: [
              ...
              Flexible(
                // Flexible agar teks panjang (jika ada) terpotong dengan aman
                child: Text(
                  tab.label,
